...
__;
        ...
        $this->js2 = <<<"__"
            ...
            $str
__;
        $this->addEventListener('[name=button1]','click','modalCall','ajax');
    }
*/
    // 新規・更新・削除モーダル表示(JS)
...
__;
        ...
        $this->js2 = <<<"__"
            ...
            $str
__;
        $this->addEventListener('','wait','modalCall','ajax');
    }
    // 新規・更新・削除モーダル表示(PHP)
...
__;
        $this->addEventListener('#jyu17','change','yubin','ajax');
    }
    //
    function yubin($data){
        //$data['yubin'];
...
$data['pr1'] = array('title' => 'お知らせ'); // ヘッダ
$data['pr2'] = true; // スタンダートDOM
$data['pr3'] = array("active"=>"home"); // ナビメニュー

//            $original_txt = openssl_decrypt($_SESSION['petty_cache'], 'AES-256-CBC', $enckey);
...
/*
echo $_SESSION['get_dat']['prm'];
$enckey = 'X{g)q1?@8VR_rBl9qj|W}A(v$%xE)Kq}F85V/V>kgJB%ni2Lf6';
            $original_txt = openssl_decrypt($_SESSION['get_dat']['prm'], 'AES-256-CBC', $enckey);
            $original_json = json_decode($original_txt);
            var_dump($original_json);
