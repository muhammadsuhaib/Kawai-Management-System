    ...
    function modalCallUd($data) {
        ...
        if ($data['table'] == 'jyu_k') {
            ...
            elseif ($data['syu'] == '管理領域コード') {
                ...
                $modal = $this->readModalSource('modal_' . $data['syu']);
                $sql = 'select * from jyu_i where jyui01=? and jyui02=? and jyui03=? and (jyui04=? or jyui04=? or jyui04=? or jyui04=?);';
                $par = array($data['cd'], $data['st'], $data['en'], '口座＿銀行コード', '口座＿支店コード', '口座＿種別', '口座＿番号');
                $stmt = $this->db->prepare($sql);
                $stmt->execute($par);
                while ($r = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $modal['jyui01'] = $r['jyui01'];
                    ...
                    $modal['jyui03'] = $r["jyui03"];
                    $modal['jyui04'] = '口座';
                    if ($r['jyui04'] == '口座＿銀行コード') {
                        $modal['jyui05'] = $r["jyui05"];
                    }elseif($r['jyui04'] == '口座＿支店コード'){
                        $modal['jyui06'] = $r["jyui05"];
                    }elseif($r['jyui04'] == '口座＿種別'){
                        $res[1][7] = $r["jyui05"];
                    }elseif($r['jyui04'] == '口座＿番号'){
                        $modal['jyui08'] = $r["jyui05"];
                    }
                    $modal['jyui_hid'] = $modal['jyui01'] . ',' . $modal['jyui04'] . ',' . $r["crdt"] . ',' . $r["jyui02"] . ',' . $r["jyui03"];
                }
                ...
                $modal['jyui05s'] = '';
                $modal['jyui06s'] = '';
                $sql = 'select * from kouza where gcd=? and scd=?;';
                $wpar = array($modal['jyui05'],$modal['jyui06']);
                $stmt = $this->db->prepare($sql);
                $stmt->execute($wpar);
                while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $knm = explode(' ', $result['nm']);
                    $modal['jyui05s'] = $knm[0] . '銀行 .#' . $modal['jyui05'];
                    $modal['jyui06s'] = $knm[1] . '支店 .#' . $modal['jyui06'];
                }
                $modal['jyui07opt'] = "<option value=''></option>" . $koumoku['口座種別'];
...
//        $this->addEventListener('#upbtn1', 'click', 'updateData', 'ajax');
    ...
    function validateData($data) {
        ...
        if (!$this->err) {
            ...
            } elseif ($data['jyui04'] == '口座') {
                $this->validate($data['jyui05'], 'str', array(0, 4, false), $data['lab05']);
                $this->validate($data['jyui06'], 'str', array(0, 3, false), $data['lab06']);
                $this->validate($data['jyui07'], 'str', array(0, 2, false), $data['lab07']);
                $this->validate($data['jyui08'], 'str', array(0, 20, false), $data['lab08']);
    ...
    function updateData($data) {
        ...
        if (!$this->err and $data['jyuk01']) { // jyuk
            ...
            if ($data['jyui04'] == '姓名') {
                ...
                $sql .= "(?,?,?,?,?,?,?,?,?,?,?,?,?,?),(?,?,?,?,?,?,?,?,?,?,?,?,?,?),(?,?,?,?,?,?,?,?,?,?,?,?,?,?),(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
                $par = array(
                    $data['jyui01'], $data['jyui02'], $data['jyui03'], '口座＿銀行コード', $data['jyui05'], '', '', '', '', '', $parwrk[2], $_SESSION['従業員コード'], $now, $_SESSION['従業員コード'],
                    $data['jyui01'], $data['jyui02'], $data['jyui03'], '口座＿支店コード', $data['jyui06'], '', '', '', '', '', $parwrk[2], $_SESSION['従業員コード'], $now, $_SESSION['従業員コード'],
                    $data['jyui01'], $data['jyui02'], $data['jyui03'], '口座＿種別', $data['jyui07'], '', '', '', '', '', $parwrk[2], $_SESSION['従業員コード'], $now, $_SESSION['従業員コード'],
                    $data['jyui01'], $data['jyui02'], $data['jyui03'], '口座＿番号', $data['jyui08'], '', '', '', '', '', $parwrk[2], $_SESSION['従業員コード'], $now, $_SESSION['従業員コード'],
                );
            } else {
                $sql .= "(?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
                $par = array(
...
__;
    ...
    // 支店名から支店コード確定(JS)
    function jyui06sKeyupJs() {
        $this->clearJs();
        $this->js1 = <<<"__"
        if($('#jyui04').val()=='口座'){
            var ary = $('#jyui06s').val().split('.#');
            if(ary.length == 2){
		$('#jyui06').val(ary[1]);
            }else{
		$('#jyui06').val('');
...
__;
        $this->js2 = <<<"__"
__;
        $this->addEventListener('#jyui06s', 'keyup', '', '');
    }

    function jyui05sOnfocusJs() {
        $this->clearJs();
        $this->js1 = <<<"__"
                var ary = $('#jyui05s').val().split('.#');
            $('#jyui05').val(ary[1]);
...
__;
        $this->addEventListener('#jyui05s', 'focus', 'getBankData', 'ajax');
    }
...
__;
        $this->addEventListener('#jyui06s', 'focus', 'getBranchOfficeData', 'ajax');
    }
