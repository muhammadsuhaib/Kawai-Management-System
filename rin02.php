...
__;
    ...
    function modalCallUd($data){
    	...
    	while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
			...
			$modal['ring14ud'] = $this->h($result['ring14']);
            ...
            if($result['ring17'] == '*'){
                $modal['ringsts1ud'] = 'この稟議の決済は完了しています';
            }elseif($result['ring17'] == ''){
                $modal['ringsts1ud'] = 'この稟議は未提出の稟議です';
            }else{
                $modal['ringsts1ud'] = 'この稟議は現在承認中です';
            }
			// 承認者かどうかチェック
			if($result['ring17'] != $_SESSION['id']){
				$modal['ring14ud_sts'] = ' readonly';
				$modal['createbtn3_sts'] = ' disabled';
				$modal['delbtn_sts'] = ' disabled';
				$modal['upbtn1_sts'] = ' disabled';
			}else{
				$modal['ring14ud_sts'] = '';
				$modal['createbtn3_sts'] = '';
				$modal['delbtn_sts'] = '';
				$modal['upbtn1_sts'] = '';
			}
...
//        if(!$this->err){$this->validate($data['ring03'] ,'str',  array(1,2,true), $data['lab03']);} // 承認ルート
...
//        if(!$this->err){$this->validate($data['ring13'] ,'str',  array(0,0,false), $data['lab13']);} // 
        if(!$this->err){$this->validate($data['ring14'] ,'str',  array(0,1000,false),  $data['lab14']);} // 一時コメント保存
...
//        if(!$this->err){$this->validate($data['ring20'] ,'str',  array(0,500,false), $data['lab20']);} // コメント（蓄積）		
		...
		$stmt->execute($par);
		while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
            if($result['ring17'] != $_SESSION['id']){ // 現在承認者でない
				$this->err = '現在承認者ではないためデータを更新できません。';
			}
        }
/*			
        // 添付ファイルアップロードしなおし
...
*/
		...
		if(!$this->err){
        	...
        	array_push($par,$today,$now,$_SESSION['id'],$data['ring01']);
        	$stmt = $this->db->prepare($sql);
...
__;
    ...
    function createLockDataModal($data){
        ...
        $modal['kakunin'] = "この稟議を承認しますか？<br>（次の承認者に承認依頼メールが送信されます）";
        $res[0]['html'] = implode($modal);
        echo json_encode($res);
        exit();
    }
...
__;
    ...
    function createLockData($data){
		...
		while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
			...
			$ring14 = $this->h($data['ring14']); // コメントはPOST取得した最新のもの
			$ring15 = $result['ring15']; // 承認ルート
			$ring16 = $result['ring16']; // 未承認者
			$ring17 = $result['ring17']; // 現在承認者CD
...
*/            
        ...
        if(!$this->err){
			$this->validate($data['ring14'] ,'str',  array(0,1000,false),  $data['lab14']);
			if($ring16jsn[0]['id'] != $_SESSION['id']){
				$this->err = '承認ルートによるとあなたが承認者ではありません。';
			}
		}
		if($this->err){
            $modal = $this->readModalSource('modal_n');
        ...
        $ring15disp = $wrkstr;
		// 未承認ルート
		array_shift($ring16jsn); // 先頭要素（=現承認者）を削除して配列を詰める
        $wrkstr = '';
		...
		if(!$ring16jsn){ // 最終承認
			...
			$ring16 = json_encode($ring16jsn); // 未承認者
			$ring17 = $ring16jsn[0]['id']; // 現在承認者CD
			$ring18 = $ring16jsn[0]['name']; //	現在承認者名
			$mail_nxt = $ring16jsn[0]['mail']; //	現在承認者メール
		}
		
        $today = date('Ymd');
        $now = date('Hi');
        $par = array($ring14,$ring16,$ring17,$ring18,$ring20,$today,$now,$_SESSION['id'],$ring01);
        $sql = "update ringi set ring14=?,ring16=?,ring17=?,ring18=?,ring20=?,uday=?,utim=?,uid=? where ring01=?";
        $stmt = $this->db->prepare($sql);
        $wrk[0]['res'] = $stmt->execute($par);
		if($wrk[0]['res']){
            if($ring07 == '1'){
			...
			if($ring17 == '*'){ // 関係者全員
				...
				$sql = 'select jyuc05 from jyu_c where jyuc04 = "kmail" and jyuc01 = ? and jyuc02 <= ? and jyuc03 >= ? ;';
				$par = array($ring04,$today,$today);
				$stmt = $this->db->prepare($sql);
        		$stmt->execute($par);
				while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
					$to[] = $result['jyuc05'];
...
/////////////////////////////////////////////////
				...
				$title = "【通知】稟議No.{$ring01} 決済完了";
            	$content = <<<"__"
to : {$todum}
下記稟議が最終承認されましたのでお知らせ致します。

-----------------------------------------
No.{$ring01}　　　起案日：{$ring02}
承認ルート：{$ring15disp}
タイトル　：{$ring05}
...
__;
    ...
    function delmodal($data){
		...
		$modal['kyakka'] = 'この稟議を却下しますか？<br>（起案者および承認完了者に却下通知メールが送信され、稟議は提出前の状態に戻ります）';
		$res[0]['html'] = implode($modal);
        echo json_encode($res);
        exit();
    }
...
//		array_shift($ring16jsn); // 先頭要素（=現承認者）を削除して配列を詰める
		...
		if($wrk[0]['res']){
            ...
            $to = array_diff($array15, $array16); // 承認完了者
...
/////////////////////////////////////////////////
	        ...
	        $content = <<<"__"
to : {$todum}
下記稟議は却下されましたのでお知らせ致します。
確認して再提出（又は削除）をお願い致します。
...
__;
    ...
    function searchModalCall($data){
        ...
        $sql = "select * from kensakupt where kns01=? and kns05='ringiS' order by kns02 DESC;";
        $stmt = $this->db->prepare($sql);
...
__;
    ...
    function searchData($data){
        ...
        $i = 0;
        $today = date('Ymd');
        $sql = "select *,ringi.cday as cday,ringi.ctim as ctim,ringi.cid as cid,ringi.uday as uday,ringi.utim as utim,ringi.uid as uid,concat(tbl1.jyuk04,' ',tbl1.jyuk05) as cname,concat(tbl2.jyuk04,' ',tbl2.jyuk05) as uname from ringi left join jyu_k as tbl1 on tbl1.jyuk09='name' and tbl1.jyuk02<=? and tbl1.jyuk03>=? and tbl1.jyuk01=ringi.cid";
        $sql .= " left join jyu_k as tbl2 on tbl2.jyuk09='name' and tbl2.jyuk02<=? and tbl2.jyuk03>=? and tbl2.jyuk01=ringi.uid";
		if($_SESSION['rinkengen'] == 'rin-all'){
			$par = array($today,$today,$today,$today);
        	$sql .= " where ring01>0 $sqlwhere $orderby;";
		}else{
			$par = array($today,$today,$today,$today,$_SESSION['id']);
        ...
        while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            if($result['ring17'] == ''){
                $jyoukyou = '';
            }elseif($result['ring17'] == $_SESSION['id']){
                $jyoukyou = '<font color="red">※要承認※</font>';
            }elseif($result['ring17'] == '*'){
                $jyoukyou = '【完了】';
            }else{
                $jyoukyou = '承認待ち';
...
__;
    ...
    function createPattern($data){
        ...
        if($data['ptnm']) {
            ...
            $wrkcd = '';
            $par = array($_SESSION['id']);
            $sql = "select max(kns02)+1 as ptcd from kensakupt where kns01=? and kns05='ringiS' group by kns01;";
            $stmt = $this->db->prepare($sql);
            $stmt->execute($par);
            while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
                $wrkcd = $result['ptcd'];
            ...
            $sql = substr($sql,0,-1);
            $sql .= ",?,?,?,?,?,?);";
            array_push($par,$_SESSION['id'],$wrkcd,$data["ptnm"],$data["ptdef"],'ringiS',$today,$now,$_SESSION['id'],$today,$now,$_SESSION['id']);
            $stmt = $this->db->prepare($sql);
            $wrk[0]['res'] = $stmt->execute($par);
			$modal = $this->readModalSource('modal_n');
            if($wrk[0]['res']){
...
__;
    ...
    function patternRead($data){
        $sql = "select * from kensakupt where kns01=? and kns02=? and kns05='ringiS';";
        $stmt = $this->db->prepare($sql);
...
__;
    ...
    function patdeleteData($data){
        $today = date('Ymd');
        $now = date('Hi');
        $sql = "delete from kensakupt where (kns01=? and kns02=?) and kns05='ringiS';";
        $par = array($_SESSION['id'],$data['kns02']);
        $stmt = $this->db->prepare($sql);
        $wrk[0]['res'] = $stmt->execute($par);
        echo json_encode($wrk);
