    ...
    function searchData($data) {
        ...
        for ($i = 2; $i <= 4; $i++) {
            if ($data["jyouken$i"] != '' or $i == 1) {
                ...
                $sqlwhere .= $data["maekakko$i"] . '(jyui04="' . $data["koumoku$i"] . '" and ';
                if ($data["enzan$i"] == 'in' or $data["enzan$i"] == 'not in') {
                    $val = explode(',', $data["val$i"]);
                    $sqlwhere .= 'jyui05 ' . $data["enzan$i"] . ' (';
                    foreach ($val as $sval) {
                        $sqlwhere .= "'$sval',";
                    }
                    ...
                    $val = explode(',', $data["val$i"]);
                    $sqlwhere .= 'jyui05 ' . $data["enzan$i"] . ' ' . "'$val[0]'" . ' and ' . "'$val[1]')" . $data["atokakko$i"];
                } elseif ($data["enzan$i"] == 'like') {
                    $val = '%' . $data["val$i"] . '%';
                    $sqlwhere .= 'jyui05 ' . $data["enzan$i"] . ' ' . "'$val')" . $data["atokakko$i"];
                } else {
                    $val = $data["val$i"];
                    $sqlwhere .= 'jyui05 ' . $data["enzan$i"] . ' ' . "'$val')" . $data["atokakko$i"];
                }
        ...
        $sql = "select jyuk01 from jyu_k";
        $sql .= " left join jyu_i on jyuk01=jyui01 and jyui02<='{$data['val1']}' and jyui03>='{$data['val1']}'";
        $sql .= " where jyuk02<='{$data['val1']}' and jyuk03>='{$data['val1']}' $sqlwhere group by jyuk01;";
        $stmt = $this->db->prepare($sql);
        $stmt->execute($par);
        while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $w = $result['jyuk01'] . ',';
        ...
        if ($w) {
            ...
            $sql .= " left join jyu_i on jyuk01=jyui01 and jyui02<='{$data['val1']}' and jyui03>='{$data['val1']}'";
            $sql .= " where jyuk02<='{$data['val1']}' and jyuk03>='{$data['val1']}' and jyuk01 in $w group by jyuk01 $orderby;";
            $stmt = $this->db->prepare($sql);
            $stmt->execute($par);
            while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
